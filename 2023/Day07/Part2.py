...
for hand in hands:
    ...
    jokers = Counter(cards)[1]
    cardcount = list(Counter([x for x in cards if x != 1]).values())

    # five of a kind
    if 5 - jokers in cardcount or jokers == 5:
        fiveofakind.append(hand)
        continue

    # four of a kind
    if 4 - jokers in cardcount:
        fourofakind.append(hand)
        continue

    # full house
    if (3 in cardcount and 2 in cardcount or
        [2, 2] == cardcount and jokers == 1 or
        2 in cardcount and jokers == 2):
        fullhouse.append(hand)
        continue

    # tree of a kind
    if 3 - jokers in cardcount:
        treeofakind.append(hand)
        continue

    # two pairs
    if (2 in Counter(cardcount).values() or
        2 in cardcount and jokers == 1):
        twopairs.append(hand)
        continue

    # one pair
    if 2 in cardcount or jokers > 0:
        onepair.append(hand)
        continue
